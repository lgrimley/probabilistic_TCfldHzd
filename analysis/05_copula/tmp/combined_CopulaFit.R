...
} else if (fit$familyname == 't'){
  copula <- tEVcop #copBasic
}

joint_probs_or <- joint.curvesCOP(cop=copula, para = c(fit$par,fit$par2),
                                  type = 'and', probs = target_prob)
joint_probs_and <- joint.curvesCOP(cop=copula, para = c(fit$par,fit$par2), 
...
p1a <- ggplot() +
  ...
  geom_line(data = line_data_100, aes(x = x, y = y), 
            color = 'red', size = 1, linetype = "longdash", alpha = 0.8) +
