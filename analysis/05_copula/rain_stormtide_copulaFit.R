...
for (b in basins){
  ...
  if (fit$familyname == 'Survival Joe'){
    copula <- JOcopB5 #Vinecopula
  } else if (fit$familyname == 'Independence'){
    copula <- P #copBasic
  } else if (fit$familyname == 'Survival Clayton'){
  ...
  print(names(joint_probs_or))
  print(names(joint_probs_and))
  
  # densities of joint probabilities 
  # to find "most likely" point on return period curve
  pdf_100yr_and <- BiCopPDF(u1=joint_probs_and$`0.997729408735735`[,1], 
                            u2=joint_probs_and$`0.997729408735735`[,2], 
                            family=fit$family, 
                            par=fit$par)
  
  pdf_100yr_or <- BiCopPDF(u1=joint_probs_or$`0.997729408735735`[,1], 
                           u2=joint_probs_or$`0.997729408735735`[,2], 
                           family=fit$family, 
                           par=fit$par)
  
  
  pdf_25yr_and <- BiCopPDF(u1=joint_probs_and$`0.953701328422112`[,1], 
                            u2=joint_probs_and$`0.953701328422112`[,2], 
                            family=fit$family, 
                            par=fit$par)
  
  pdf_25yr_or <- BiCopPDF(u1=joint_probs_or$`0.953701328422112`[,1], 
                           u2=joint_probs_or$`0.953701328422112`[,2], 
                           family=fit$family, 
                           par=fit$par)
  ...
  x_100yr_and <- qssd(p=joint_probs_and$`0.997729408735735`[,1], 
                          param=x_marg)
  y_100yr_and <- qssd(p=joint_probs_and$`0.997729408735735`[,2], 
                          param=y_marg)
  
  x_25yr_and <- qssd(p=joint_probs_and$`0.953701328422112`[,1], 
                      param=x_marg)
  y_25yr_and <- qssd(p=joint_probs_and$`0.953701328422112`[,2], 
                      param=y_marg)
  ...
  p <- ggplot() +
    geom_line(aes(x=x_100yr_and, y=y_100yr_and), color='black', size=1.5) +
    geom_line(aes(x=x_25yr_and, y=y_25yr_and), color='black', size=1.5) +
    ...
    geom_point(data = df_outline, 
               aes(x = stormtide, y = maxRR), 
               shape = 21, fill = NA, color = "black", stroke = 1.2, size = 4) +
    scale_color_manual(values = blue_shades) +
    
    labs(y='Rainfall Intensity (mm/hr)',
         x='Storm Tide Peak (m)',
         title=paste0(b,' ',climate, ' (', fit$familyname, ')'),
         color = "Compound\nExtent\nReturn Period",
         ) + 
    theme_minimal() +
    theme(
...
for (b in basins){
  ...
  # densities of joint probabilities 
  # to find "most likely" point on return period curve
  pdf_100yr_and <- BiCopPDF(u1=joint_probs_and$`0.997620017763334`[,1], 
                            u2=joint_probs_and$`0.997620017763334`[,2], 
                            family=fit$family, 
                            par=fit$par)
  
  pdf_100yr_or <- BiCopPDF(u1=joint_probs_or$`0.997620017763334`[,1], 
                           u2=joint_probs_or$`0.997620017763334`[,2], 
                           family=fit$family, 
                           par=fit$par)
  
  
  pdf_25yr_and <- BiCopPDF(u1=joint_probs_and$`0.951470783107989`[,1], 
                           u2=joint_probs_and$`0.951470783107989`[,2], 
                           family=fit$family, 
                           par=fit$par)
  
  pdf_25yr_or <- BiCopPDF(u1=joint_probs_or$`0.951470783107989`[,1], 
                          u2=joint_probs_or$`0.951470783107989`[,2], 
                          family=fit$family, 
                          par=fit$par)
  ...
  # find values corresponding to quantiles from joint return period curves
  # using fBasics package; Returns spline smoothed quantile function
  x_100yr_and <- qssd(p=joint_probs_and$`0.997620017763334`[,1], 
                      param=x_marg)
  y_100yr_and <- qssd(p=joint_probs_and$`0.997620017763334`[,2], 
                      param=y_marg)
  
  x_25yr_and <- qssd(p=joint_probs_and$`0.951470783107989`[,1], 
                     param=x_marg)
  y_25yr_and <- qssd(p=joint_probs_and$`0.951470783107989`[,2], 
                     param=y_marg)
