...
os.chdir(r'Z:\Data-Expansion\users\lelise\projects\Carolinas_SFINCS\Chapter3_SyntheticTCs\04_MODEL_OUTPUTS\slr_runs\canesm_ssp585_SRL112cm\hmin0.05')


df = pd.read_csv('canesm_slr_sbg20m_hmin0.05.csv', index_col=0)
df['tc_id'] = df.index.split('_')[0]
noslr_df = df[~df['tc_id'].str.contains('SLR', case=False, na=False)]
noslr_df['Period'] = 'No SLR'

slr_df = df[df['tc_id'].str.contains('SLR', case=False, na=False)]
slr_df['Period'] = 'SLR'

select_columns = ['basin','tc_id','Period','Coastal_Area_sqkm','Compound_Area_sqkm','Runoff_Area_sqkm','Total_Area_sqkm']
noslr_df = df[select_columns]
noslr_df.columns = ['AOI', 'tc_id', 'Period', 'Coastal', 'Compound', 'Runoff', 'Total']

slr_df = df[select_columns]
slr_df.columns = ['AOI', 'tc_id', 'Period', 'Coastal', 'Compound', 'Runoff', 'Total']
...
futdf.columns = ['AOI','tc_id', 'Period', 'Coastal', 'Compound', 'Runoff', 'Total', 'weight']
scenarios = ['Runoff', 'Coastal', 'Compound', 'Total']
df1 = expand_df_by_weights(df=futdf, scenarios=scenarios)


violin_plot_biascorrected = True
...
hist_group = histdf.groupby('AOI')[['Compound','Coastal', 'Runoff','Total']].describe(percentiles=[0.05, 0.1, 0.25, 0.50, 0.75, 0.90, 0.95])
hist_group = hist_group.T
t = histdf[histdf['AOI']=='Domain']
proj_group = df1.groupby('AOI')[['Compound','Coastal', 'Runoff','Total']].describe(percentiles=[0.05, 0.1, 0.25, 0.50, 0.75, 0.90, 0.95])
...
combined_stats.to_csv(r'flood_extent_stats_sbg20m.csv')


# Create a table of stats
a = histdf.groupby('AOI')[['Compound','Coastal', 'Runoff','Total']].mean()
a.columns = [f'{s}_mean' for s in a.columns]
b = histdf.groupby('AOI')[['Compound','Coastal', 'Runoff','Total']].std()
b.columns = [f'{s}_std' for s in b.columns]
c = histdf.groupby('AOI')[['Compound','Coastal', 'Runoff','Total']].mean()
c.columns = [f'{s}_mean_Fut' for s in c.columns]
d = histdf.groupby('AOI')[['Compound','Coastal', 'Runoff','Total']].std()
d.columns = [f'{s}_std_Fut' for s in d.columns]
abcd = pd.concat(objs = [a,b,c,d], axis=1, ignore_index=False).round(2)
abcd.to_csv(r'flood_area_standardDev2.csv')
