...
with open(f'python_output_file.txt', 'a') as python_output_file:
    ...
    # Read stdout and stderr in real-time
    line_entry = f'Running {track_id} with Slurm Job Array Task ID {task_id}\n'
    python_output_file.write(line_entry)  # Write output to log file
    python_output_file.flush()  # Ensure the log file is flushed immediately
    ...
    python_output_file.write(f'Completed run moved to: {destination_folder}\n')
    python_output_file.flush()  # Ensure the log file is flushed immediately
