...
wl_gage_ids = pd.read_csv(r'Z:\Data-Expansion\users\lelise\projects\Carolinas_SFINCS\Chapter3_SyntheticTCs\02_DATA\NCEP_Reanalysis\stormTide\coastline_lores_NCSC.csv')
...
stormTide_dir = os.path.join(os.getcwd(), 'stormTide')
track_dir = os.path.join(os.getcwd(), 'tracks')
track_file = os.path.join(track_dir,'UScoast6_AL_ncep_reanal_roEst1rmEst1_trk100')
stormTide_file = os.path.join(stormTide_dir,'ncep_reanal_WLseries.mat')
...
for gcm in gcm_runs:
    ...
    wl_inds = da.from_array(wl_data_mat['WL_inds_adj'])
    ...
    for tc_id in storms['tc_id']:
        ...
        all_zeros = storm_wl.eq(0).all().all()
        if all_zeros == np.True_:
            print(f'No ADCIRC surge output for storm: {tc_id}. Skipping this storm.')
            continue
        else:
